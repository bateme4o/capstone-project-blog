...
-- Get admin's user_profiles.id (needed for author_id foreign key)
-- Run this first to see the user_profiles.id:
-- SELECT id FROM public.user_profiles WHERE user_id = 'eeb82ae8-9102-4bf7-a9b5-58b0107c5848';
-- Then replace ADMIN_PROFILE_ID below with the result.

-- For now, we'll insert posts with author_id = NULL, then update after creating user_profiles
...
We''ll cover the fundamentals of HTML, CSS, and JavaScript - the three pillars of web development. By the end of this guide, you''ll have a solid foundation to build upon.',
  'https://via.placeholder.com/600x300?text=Placeholder',
  'published',
  now(),
  NULL,
  now(),
...
-- Alice's posts: Web Development, JavaScript
...
-- Bob's posts: Database, API, Testing
INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111113', id FROM public.tags WHERE name IN ('Database', 'Backend')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111114', id FROM public.tags WHERE name IN ('API', 'Backend')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111115', id FROM public.tags WHERE name IN ('Testing', 'Best Practices')
ON CONFLICT DO NOTHING;

-- Carol's posts: CSS, Frontend
INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111116', id FROM public.tags WHERE name IN ('CSS', 'Frontend')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111117', id FROM public.tags WHERE name IN ('Frontend', 'Web Development')
ON CONFLICT DO NOTHING;

-- David's posts: DevOps, Tools
INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111118', id FROM public.tags WHERE name IN ('Best Practices', 'Tutorial')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111119', id FROM public.tags WHERE name IN ('DevOps', 'Backend')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111120', id FROM public.tags WHERE name IN ('DevOps', 'Backend')
ON CONFLICT DO NOTHING;

-- Emma's posts: Machine Learning, Security
INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111121', id FROM public.tags WHERE name IN ('Tutorial', 'Backend')
ON CONFLICT DO NOTHING;

INSERT INTO public.article_tags (article_id, tag_id)
SELECT 'a1111111-1111-1111-1111-111111111122', id FROM public.tags WHERE name IN ('Security', 'Best Practices')
ON CONFLICT DO NOTHING;
...
--
-- Test Users Created:
-- 1. Alice Johnson (alice@example.com) - User role, 2 posts
-- 2. Bob Smith (bob@example.com) - Admin role, 3 posts
-- 3. Carol White (carol@example.com) - User role, 2 posts
-- 4. David Brown (david@example.com) - User role, 3 posts
-- 5. Emma Davis (emma@example.com) - User role, 2 posts
--
-- Total: 5 users, 12 test posts
--
-- Note: Test user auth accounts need to be created in Supabase Auth separately.
-- Use the Supabase dashboard or Auth API to create accounts with these emails.
--
